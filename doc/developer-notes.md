...
- Basic rules specified in src/.clang-format. Use a recent clang-format-3.5 to format automatically.
  - Blunexs on new lines for namespaces, classes, functions, methods.
  - Blunexs on the same line for everything else.
...
Lunex Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 10050.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 10051 for connections and services them.
...
However, in open source software such as Lunex Core, where everyone uses
...
pay attention to for reviewers of Lunex Core code.
...
- Never use the `std::map []` syntax when reading from a map, but instead use `.find()`
  ...
  - *Rationale*: `[]` does an insert (of the default element) if the item doesn't
    ...
    lunex conditions (expecting read-read behavior). Using `[]` is fine for *writing* to a map
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Lunex Core uses tinyformat, which is type safe. Leave them out to avoid confusion
