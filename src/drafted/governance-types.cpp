...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "LunexNetwork") type = LunexNetwork;
    if(strType == "LunexNetworkVariable") type = LunexNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == LunexNetwork) s = "LunexNetwork";
    if(type == LunexNetworkVariable) s = "LunexNetworkVariable";
