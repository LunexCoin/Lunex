...
#if defined(NDEBUG)
# error "Lunex Core cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in nLUNEX) are considered zero fee (for relaying, mining and transaction creation) */
...
const string strMessageMagic = "LunexCoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    // yearly decline of production by 80% per year until reached max coin
    for (int i = consensusParams.nSubsidyHalvingInterval; i <= nPrevHeight; i += consensusParams.nSubsidyHalvingInterval) {
        nSubsidy -= nSubsidy/80;
    }
...
{
    CAmount ret = blockValue/1.25; // 80%

    int nMNPIBlock = Params().GetConsensus().nMasternodePaymentsIncreaseBlock;
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;

                                                                      // mainnet:
    if(nHeight > nMNPIBlock)                  ret += blockValue / 20; // 25000  - 85.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 1)) ret += blockValue / 50; // 38000  - 86.25%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 2)) ret += blockValue / 50; // 51000  - 87.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 3)) ret += blockValue / 50; // 64000  - 88.75%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 4)) ret += blockValue / 50; // 77000  - 90.0%
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // LUNEX : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(LUNEX): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(LUNEX): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // LUNEX : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(LUNEX): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(LUNEX): spork is off, skipping transaction locking checks\n");
    }
