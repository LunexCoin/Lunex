...
//#define ENABLE_LUNEX_DEBUG
#ifdef ENABLE_LUNEX_DEBUG
#define DBG( x ) x
#else
#define DBG( x ) 
#endif
 ...
 */
template <typename Callable> void TlunexThread(const char* name,  Callable func)
{
    std::string s = strprintf("lunex-%s", name);
