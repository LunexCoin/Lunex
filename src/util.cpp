...
const char * const BITCOIN_CONF_FILENAME = "lunex.conf";
const char * const BITCOIN_PID_FILENAME = "lunexd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "lunex" is a composite category enabling all Lunex-related debug output
            if(ptrCategory->count(string("lunex"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LunexCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LunexCore
    // Mac: ~/Library/Application Support/LunexCore
    // Unix: ~/.lunexcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LunexCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LunexCore";
#else
    // Unix
    return pathRet / ".lunexcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty lunex.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override lunex.conf
